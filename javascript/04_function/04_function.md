...
* 创建一个新的对象`obj`
* 将对象与构建函数通过原型链接起来
* 将构建函数的`this`绑定到新建的对象obj上
* 如果构造器内部没有自己return一个自定义对象，则返回上述构建对象；否则返回自定义对象。

---

## <a id="9">原型链</a>
`js`常被描述为一种基于原型的语言。  
当试图访问一个属性时，如果该对象没有该属性，会顺着原型、原型的原型，依次层层向上搜索。  
内置对象有prototype属性，而创建的实例对象需要用__proto__属性来访问原型。  
下面举个例子：
...
分析如下：
* 构造函数`Person`存在一个原型，可以用`prototype`属性访问;
* 通过构造函数`Person`生成了实例对象`person`, `person.__proto__`指向构造函数`Person`的`prototype`;
